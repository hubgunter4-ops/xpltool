...
# Compatibilidad con la API pública del script anterior.
TOOL_BINARIES = {"metasploit": "msfconsole"}
...
def cc(text, code):
    return f"{code}{text}{Colors.RESET}" if USE_COLOR else str(text)
...
def check_and_install_tools(selected=None, interactive=True):
    ...
    if missing and interactive:
        try:
            install_requested = confirm("¿Instalar herramientas faltantes?", "s")
        except (EOFError, KeyboardInterrupt):
            print()
            info("Sin confirmación interactiva; no se instalarán herramientas.")
...
def validate_target(target):
    """Compatibilidad con el validador público de la primera versión."""
    return Security.validate_target(target)


def generate_wordlist(url, output_path, depth=2, min_len=5, session_dir=None):
    """Genera una wordlist con cewl conservando la interfaz anterior."""
    section("Generación de Wordlist Personalizada (cewl)")
    if not tool_installed("cewl"):
        warn("cewl no disponible.")
        return False
